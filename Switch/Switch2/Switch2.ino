...
bool onPowerState(const String &deviceId, bool &state) {
  ...
  digitalWrite(RELAY_01, myPowerState?LOW:HIGH);
...
void handleButtonPress() {
  ...
  if (digitalRead(BTN_FLASH) == LOW && actualMillis - lastBtnPress > 1000)  { // is button pressed (inverted logic! button pressed = LOW) and debounced?
    ...
    digitalWrite(RELAY_01, myPowerState?LOW:HIGH); // if myPowerState indicates device turned on: turn on led (builtin led uses inverted logic: LOW = LED ON / HIGH = LED OFF)
...
void setup() {
  ...
  digitalWrite(RELAY_01, HIGH); // turn off LED on bootup
