...
bool onPowerState(const String &deviceId, bool &state) {
  ...
  myPowerState = state;
  digitalWrite(relayinput, myPowerState?LOW:HIGH);
...
void handleButtonPress() {
    ...
    //delay(1000);
    digitalWrite(relayinput, LOW);// if myPowerState indicates device turned on: turn on led (builtin led uses inverted logic: LOW = LED ON / HIGH = LED OFF)
...
void setup() {
 // pinMode(BTN_FLASH, INPUT_PULLUP); // GPIO 0 as input, pulled high
  pinMode(relayinput, OUTPUT); // define LED GPIO as output
