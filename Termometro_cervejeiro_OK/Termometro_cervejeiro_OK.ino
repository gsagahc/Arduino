...
const int BotaoUp = 10; //porta do botão que aumenta
const int BotaoDown = 11;// porta do botão que diminui
...
void loop()
{
  ...
  // Mostra dados no LCD  
    ...
    //se temperatura <=7 desliga relé
    if (tempC <= tempMin)
    {
     digitalWrite(porta_rele1, HIGH);  //Desliga rele 1
    }
    //se temperatura >=10 liga relé
    if (tempC >= tempMax)
    {
 ...
 /* if((digitalRead(BotaoUp) == HIGH) || (digitalRead(BotaoDown) == HIGH))
  {
     delay(vdelay);    
  }*/
